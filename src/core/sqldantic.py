...
from typing import (get_origin, get_args, 
                    Any, Type, TypeVar, TypeAlias,
                    Union, Annotated, Optional, _SpecialForm, 
                    Callable, Iterator, Literal, LiteralString,
                    Tuple, List, Set,
                    Sequence, Collection,
                    Dict, Mapping)
...
BaseModelInstanceOrType = Union[BaseModel, BaseModelType] # So tables can be created from instances of models or the models themselves
...
TableNameTransformer = Union[Callable, Sequence[Callable]] # Function or list of functions to transform table name (e.g. lowercase, pluralize, add some prefix, etc.)
PrimaryKeyGetterOrFieldName = Union[Callable, str] # Function or field name to derive primary key from model (e.g. first field, field with name 'id', etc.)
...
def get_table_name(model: BaseModelType, 
...
def get_primary_key(model: BaseModelType, 
                    primary_key_getter: PrimaryKeyGetterOrFieldName,
                    primary_key_getter_args: OptionalArgs) -> str:
...
def is_type_and_subclass(annotation: Union[type, SQLDanticType, None], _class_or_tuple: Union[type, Tuple[type]]) -> bool:
...
def get_sqldantic_type(annotation: Union[type, SQLDanticType, None],
    ...
    if origin in (Annotated, Union) or isinstance(type(origin), _SpecialForm): #is subclass?
        for arg in get_args(annotation):
            if arg is None:
                continue
            elif ((sqldantic_type_and_item_type := get_sqldantic_type(arg))[0] not in ("UNION", "ANNOTATED")):
...
def get_model_sqldantic_fields(model: BaseModelType,
                               sqldantic_type_map: SQLDanticTypeMap = SQLDANTIC_TYPE_MAP
                               ) -> Dict[str, SQLDanticFieldType]:
    return {field_name: get_sqldantic_type(field_info.annotation, sqldantic_type_map) 
...
def catch_sqlite_errors(func):
    ...
    def wrapper(instance, *args, **kwargs):
        try:
            ...
            if (instance.ignore_sqlite_errors
                and (
                    ...
                    or (
                        (error_str := str(e)) 
                        and any(error_str.startswith(ignored_error)
...
class BaseDB:
    ...
    @catch_sqlite_errors
    def recusive_create_table_queries(self, 
                                      model: BaseModelInstanceOrType, 
        ...
        column_defs = []
        foreign_keys = []
        for field_name, sqldantic_type_and_item_type in sqldantic_schema.sqldantic_fields.items():
            ...
            if is_base_model(sqldantic_type):
                ...
                foreign_keys.append(f"FOREIGN KEY ({field_name}) REFERENCES {sub_schema.table_name} ({sub_schema.primary_key})")
    ...
    def create_tables_from_model(self, 
                                 model: BaseModelInstanceOrType,
                                 if_not_exists: bool = True
                                 ) -> None:
    ...
    @catch_sqlite_errors
    def recursive_modify_table_queries(self,
                                model: BaseModel,
                                query_facory: Callable
                                ) -> Iterator[Query]:
        ...
        for field_name, sqldantic_type_and_item_type in sqldantic_schema.sqldantic_fields.items():
            ...
            if value is None:
                ...
                if is_base_model(item_type):
                    ...
                    model_dict[field_name] = self.SEQUENCE_SEPARATOR.join(str(item) if not isinstance(item, str) else item for item in sequence_values)

                elif value is None:
                    model_dict[field_name] = None
                else:
                    model_dict[field_name] = self.SEQUENCE_SEPARATOR.join(str(item) if not isinstance(item, str) else item for item in value)
    ...
    def iter_models(self, 
        ...
        for row in self.cursor.fetchall():
            model_dict = {}
            for row_index, (field_name, sqldantic_type_and_item_type) in enumerate(sqldantic_schema.sqldantic_fields.items()):
                sqldantic_type, item_type = sqldantic_type_and_item_type
                value = row[row_index]
                if value is None:
                    model_dict[field_name] = None
                    ...
                    try:
                        model_dict[field_name] = next(self.iter_models(
                            sqldantic_type,
                            value,
                    ...
                    if is_base_model(item_type):
                        model_dict[field_name] = list(self.iter_models(
                            item_type, 
                            *sequence_values, 
                            select_query_factory=select_query_factory,
                            **select_query_factory_kwargs
    ...
    def get_models(self,
        ...
        return list(self.iter_models(
            model, *select_query_factory_args, select_query_factory=select_query_factory, **select_query_factory_kwargs))

    def get_models_by_primary_key(self, 
                                  model: Union[_BaseModel, Type[_BaseModel]], 
                                  *primary_keys: str
                                  ) -> List[_BaseModel]:
    ...
    def get_model(self,
        ...
        try:
            return next(self.iter_models(
                model, *select_query_factory_args, select_query_factory=select_query_factory, **select_query_factory_kwargs))
        except StopIteration:
            return None

    def get_model_by_primary_key(self, 
                                 model: Union[_BaseModel, Type[_BaseModel]], 
                                 *primary_keys: str
                                 ) -> Optional[_BaseModel]:
