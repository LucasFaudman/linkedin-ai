...
class LinkedInAutomator:
    ...
    def __init__(self,
        ...
        self.job_app_db_path = job_app_db_path if isinstance(
            job_app_db_path, Path) else Path(job_app_db_path)       
    ...
    def init_scraper(self):
        ...
        self.scraper = SouperScraper(
            executable_path=self.webdriver_path, user_agent=self.user_agent, proxy=self.proxy)
        ...
        if post_apply_content := soup.find('div', attrs={'class': 'post-apply-timeline__content'}):
            for post_appy_entity in post_apply_content.find_all('li', attrs={'class': 'post-apply-timeline__entity'})[::-1]:
                ...
                time = post_appy_entity.find(
                    'span', attrs={'class': 'post-apply-timeline__entity-time'}).text.strip()
                    ...
                    for upload_elm in upload_elms:
                        if 'Upload cover letter' in upload_elm.text:
                            if self.cover_letter_action == "skip":
                                ...
                                cover_letter_path = self.generate_cover_letter(
                                    job)
                                upload_success = self.upload_cover_letter(
                                    cover_letter_path)
    ...
    def upload_cover_letter(self, cover_letter_path: str | Path) -> bool:
        ...
        cover_letter_path = cover_letter_path if isinstance(
            cover_letter_path, str) else str(cover_letter_path)
    ...
    def get_questions(self) -> Iterator[tuple[WebElement | dict[str, WebElement], Question]]:
